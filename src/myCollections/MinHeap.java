...
public class MinHeap<T extends Comparable<? super T>> {
	...
	public void add(T x)
	{
		...
		//Used to move the element up the tree if its parent is bigger than it.
		percolateUp(hole);
	}
	 ...
	 * If the child is smaller than the parent, they are swapped in the array.
	...
	private void percolateUp(int hole) 
	{
		/* Iterates while the hole is greater than 0, meaning it reached the root
		   and while the parent is larger than the child.
	 ...
	 * It chooses the smallest of its children and if the parent is smaller than the child, swaps them.
	...
	private void percolateDown(int index)
	{
		...
		while((2 * index) + 2 < heap.size())
		{
			...
			//If the left child is smaller than both the parent and the right child, then swap it with the current parent.
			...
			//If the right child is smaller than both the parent and the let child, then swap it with the current parent.
			...
			//If both children were larger, then the parent is in the right spot, so return.
			else
				return;
		}
	}
